...
use App\Models\SeminarSix;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;
use Illuminate\Support\Facades\DB;


class SeminarSixController extends Controller
{
    // Function retiverd data from `clien_data` table
    public function index()
    {
        $clients = SeminarSix::all();
    ...
    public function register(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'client_email' => 'required|email|unique:seminar_sixes',
            'client_phone' => 'required|unique:seminar_sixes'
        ...
        if ($validator->fails()) {
            ...
            $registraions = SeminarSix::create($request->all());
            SeminarSixController::sendMessage($request);
            return response()->json([
    ...
    public function sendMessage(Request $request)
    {
        ...
        $message ='*إنتظرونا* الأحد القادم (14 ابريل) في *ويبنار*  *((Natural Leadership Power))* استفد من *قوة وتأثير قيادتك الفطرية*. 

👈🏻انتظرونا في البث المباشر  يوم 14 ابريل 👇🏻
https://www.youtube.com/@ELDEMELLAWYOFFICiAL

للأشتراك وتلقي الإستفسارات عبر تطبيق الزووم 👇🏻: 
https://us02web.zoom.us/j/83041111319?pwd=a2RMUHVPNEdNYkpxVHhPZXpqcUtpUT09
*في التوقيتات* :
...
https://youtu.be/lf212c1HtFA';
    ...
    {
        $client = SeminarSix::where('client_phone', $phone)->first();
        if (!$client) {
            // No data found for the given phone number
            SeminarSixController::getData($phone);
            return true;
        } else {
            return false;
        }
